...
static int esFlotante(char* cadena,int limite); //CHEQUEAR
...
static int esDni(char* cadena,int longitud);	//CHEQUEAR
...
/**
 * \brief Verifica si la cadena ingresada es un número flotante
 * \param cadena Cadena de caracteres a ser analizada
 * \return Retorna 1 (vardadero) si la cadena es flotante, 0 (falso) si no lo es o -1 (error) si hubo error.
 *
 */
 ...
 * \param mensajeError Es el mensaje de error a ser mostrado
 * \param reintentos Cantidad de reintentos
 * \return Retorna 0 si se obtuvo el numero DNI, -1 si no
 *
 */
 ...
 * \param longitud Tamaño del string donde se guardará el resultado de la función.
 * \return Retorna 0 (EXITO) si se obtiene un numero DNI, -1 (ERROR) si no.
 *
 */
static int getDni(char* pResultado,int longitud)
{
...
/**
 * \brief Verifica si la cadena ingresada es un DNI
 * \param cadena Cadena de caracteres a ser analizada
 * \param limite Tamaño del array a ser analizado
 * \return Retorna 1 (verdadero) si la cadena es DNI, 0 (falso) si no lo es y -1 en caso de error
 *
 */
static int esDni(char* cadena,int longitud)
{
	...
	if(cadena != NULL && longitud > 0)
	{
		for(i = strnlen(cadena,longitud); i > -1; i--)
		{
			if( (cadena[i] > '0' && cadena[i] < '9') ||
				(cadena[i-4] == '.' && cadena[i-8] == '.') )
			{
				retorno = 0;
			}
			else
